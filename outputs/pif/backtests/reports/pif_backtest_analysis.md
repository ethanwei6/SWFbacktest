...
- `P2` is now the strongest absolute result at 122.3%, while `P1` remains the weakest at -75.8%.
...
- `P3` still loses even though `accumulation_like` names have higher simple average forward returns (`10.5%` vs `1.5%`), which points to concentration and tail-risk drag rather than no signal at all.
- `P4` is directionally unhelpful in this sample: avoided likely-reduction names averaged `10.2%` forward return versus `3.9%` for the names it kept.
- `P5` still changes the interpretation of the whole project: when sale proceeds are retained as cash instead of being redistributed into remaining names, the strategy stays positive in absolute terms at 48.9%, but it still fails to beat `SPY`.
