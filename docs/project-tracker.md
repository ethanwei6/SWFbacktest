...
- no new hybrid production strategy was added; the project now explicitly narrows to `N6` as the strongest residual investable sleeve, the cross-fund state model as the preferred monitoring abstraction, and `S1` as a lower-beta overlay reference rather than as a flagship alpha sleeve
- an expanded institutional benchmark layer is now complete in [`data/processed/inference/institutional_benchmark_series_daily.csv`](/Users/ethanwei/Documents/Codex/2026-05-28-i-want-to-move-to-this/data/processed/inference/institutional_benchmark_series_daily.csv), [`institutional_benchmark_summary.csv`](/Users/ethanwei/Documents/Codex/2026-05-28-i-want-to-move-to-this/data/processed/inference/institutional_benchmark_summary.csv), and [`institutional_benchmark_audit.csv`](/Users/ethanwei/Documents/Codex/2026-05-28-i-want-to-move-to-this/data/processed/inference/institutional_benchmark_audit.csv)
- that layer keeps the original market benchmarks but adds `BIL` as a cash hurdle and `USMV` / `ACWV` as defensive-equity comparators, then computes Sortino, Calmar, capture ratios, positive-month rates, rolling loss windows, and recovery durations for `P2`, `P5`, `N4`, `N6`, and `S1`
- the current institutional read is that `PIF` mirroring still does not look like a strong low-risk allocator, `N4` and `N6` remain the strongest realistic sleeves even against defensive comparators, and `S1` looks materially better as a lower-beta overlay than as a benchmark-beating alpha engine
