...
BACKTEST_ROOT = PIF_ROOT / "backtests" / "p3_accumulation_tilt"
...
ELIGIBILITY_PATH = BACKTEST_ROOT / "p3at_signal_eligibility.csv"
REBALANCES_PATH = BACKTEST_ROOT / "p3at_rebalance_events.csv"
ORDERS_PATH = BACKTEST_ROOT / "p3at_orders.csv"
HOLDINGS_DAILY_PATH = BACKTEST_ROOT / "p3at_holdings_daily.csv"
PORTFOLIO_DAILY_PATH = BACKTEST_ROOT / "p3at_portfolio_daily.csv"
SUMMARY_PATH = BACKTEST_ROOT / "p3at_summary.json"
...
def load_transition_lookup(signal_rows: list[dict[str, str]]) -> dict[tuple[str, str], dict[str, str]]:
    lookup: dict[tuple[str, str], dict[str, str]] = {}
    for row in signal_rows:
        if row["signal_source"] != "transition":
            continue
        lookup[(row["report_period"], row["security_key"])] = row
    return lookup


def transition_to_bucket(signal_type: str) -> str:
    if signal_type in {"entry_observed", "likely_accumulation"}:
        return "accumulation_like"
    if signal_type == "continued_holding":
        return "neutral"
    if signal_type == "likely_reduction":
        return "reduction_like"
    return "neutral"
...
def build_signal_eligibility(
    ...
    for row in signal_rows:
        if row["signal_type"] != "full_sleeve_holding":
            continue
        ...
        transition_row = transition_lookup.get((row["report_period"], row["security_key"]))
        transition_type = transition_row["signal_type"] if transition_row else "continued_holding"
        tilt_bucket = transition_to_bucket(transition_type)
        tilt_score = SCORES[tilt_bucket]
        ...
        if map_row is None:
            ...
            inclusion_flag = "0"
            exclusion_reason = "reduction_overlay_exclusion"
        ...
        out.append(
            {
                ...
                "disclosed_market_value_usd": row["market_value_usd"],
                "disclosed_portfolio_weight": row["portfolio_weight_in_filing"],
                "transition_signal_type": transition_type,
                "tilt_bucket": tilt_bucket,
                "tilt_score": f"{tilt_score:.1f}",
                ...
                "strategy_id": "P3_ACCUMULATION_TILT",
                "rationale_label": tilt_bucket,
                "rationale_text": "Tilt the disclosed PIF sleeve toward new or accumulating names and exclude likely reductions.",
            }
        )
...
def run_backtest() -> None:
    ...
    transition_lookup = load_transition_lookup(signal_rows)
    ...
    for current_date in active_dates:
        ...
        if current_date in eligible_by_trade_date:
            ...
            rebalance_id = f"P3AT-R{rebalance_map[current_date]:03d}"
            current_signals = sorted(
                eligible_by_trade_date[current_date],
                key=lambda row: (row["issuer_name"], row["signal_id"]),
            )
            total_score = sum(to_float(row["tilt_score"]) for row in current_signals)
                ...
                orders_rows.append(
                    {
                        ...
                        "tilt_bucket": position.tilt_bucket,
                        "tilt_score": f"{position.tilt_score:.1f}",
                        "rationale_text": "Exit previous P3 tilted basket at the next rebalance close.",
                    }
                )
                sells_count += 1
            ...
            for signal in current_signals:
                ...
                raw_score = to_float(signal["tilt_score"])
                target_weight = raw_score / total_score if total_score else 0.0
                ...
                positions[signal["security_key"]] = Position(
                    ...
                    tilt_bucket=signal["tilt_bucket"],
                    tilt_score=raw_score,
                )
                orders_rows.append(
                    {
                        "rebalance_id": rebalance_id,
                        ...
                        "transition_signal_type": signal["transition_signal_type"],
                        "tilt_bucket": signal["tilt_bucket"],
                        "tilt_score": signal["tilt_score"],
                        "rationale_text": "Buy disclosed sleeve constituent with transition-based tilt weight.",
                    }
                )
                buys_count += 1
            ...
            bucket_counts = Counter(row["tilt_bucket"] for row in current_signals)
            rebalance_rows.append(
                {
                    ...
                    "raw_holding_signal_count": str(raw_signal_count),
                    "eligible_holding_signal_count": str(len(current_signals)),
                    "excluded_holding_signal_count": str(raw_excluded_count),
                    "accumulation_like_count": str(bucket_counts.get("accumulation_like", 0)),
                    "neutral_count": str(bucket_counts.get("neutral", 0)),
                    ...
                    "total_raw_score": f"{total_score:.12f}",
                    "same_day_bundle_flag": "1" if len(report_periods) > 1 else "0",
                }
            )
    ...
    summary = {
        "strategy_id": "P3_ACCUMULATION_TILT",
        "strategy_name": "PIF Accumulation Tilt",
        ...
        "eligible_bucket_counts": dict(Counter(row["tilt_bucket"] for row in eligible_rows)),
